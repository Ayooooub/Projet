...
.dropdown-menu a:hover{
  ...
  <div class="row">
    ...
    @else
        @foreach ($lands as $building)
            @if ( $building->accepted == 0)
                <div class="col-md-4">
                    <div class="card">
                        <a href="/d_buildings/{{ $building->id }}">
                            @if( $building->images->count() > 0)
                                <img class="card-img-top" src="{{ asset('storage/land_images/'. $building->images->first()->path ) }}" alt="Maison">
                            @endif
                        </a>        
                        <div class="card-body">
                            <h5 class="card-title">{{ $building->surface }}</h5>
                            <p class="card-text">Ville: {{  $building->adresse }}</p>
                            <p class="card-text">Prix: ${{  $building->prix }}</p>
                            <div class="btn-group" role="group" aria-label="Actions">
                                <form method="post" action="{{ route('lands.destroy', ['id' => $building->id]) }}">
                                    @csrf
                                    @method('delete')
                                ...
                                <form method="post" action="{{ route('lands.accept', ['id' =>  $building->id]) }}">
                                    @csrf
                                    <input type="hidden" name="id" value="{{ $building->id }}">
